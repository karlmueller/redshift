...
static float acc_timer;
float c_speed;
float acc_t_diff; 
float now;


// STEPPER_ACC is the function here
// Acceleration-limited wrapper for runSpeed.
// Writes the speed parameter to allow for smooth
void runSpeedAcc(float cmd_speed, float now)
{
  c_speed = stepper0.speed();

  if (c_speed < cmd_speed)
  {
    acc_t_diff = esp_timer_get_time() - now;
    stepper0.setSpeed(c_speed + (STEPPER_ACC*(acc_t_diff/1000000)));

  }
  //Call runSpeed at least once per tick, required to calculate speed and run stepper
  stepper0.runSpeed();

}

void ceiling_command_speed()
{
  Serial.println("Ceiling speed routine:: Motor wake");
  ...
  while (abs(c_command_val) > stick_threshold)
  {
    ...
    runSpeedAcc(speed_normalized, esp_timer_get_time());
